...
# 2. Fix permissions logs (DÉFINITIF)
echo "🔧 2/8 - Fix permissions logs..."
docker compose exec -u root app rm -f /var/www/storage/logs/laravel.log
docker compose exec -u root app touch /var/www/storage/logs/laravel.log
docker compose exec -u root app chown www-data:www-data /var/www/storage/logs/laravel.log
docker compose exec -u root app chmod 664 /var/www/storage/logs/laravel.log
echo "✅ Logs fixés"
echo ""
...
docker compose exec -u root app find /var/www/storage -type f -exec chmod 664 {} \;
docker compose exec -u root app find /var/www/storage -type d -exec chmod 775 {} \;
echo "✅ Storage fixé"
echo ""
